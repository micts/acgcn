...
# def infer(dataloader, dataset, model, device, model_instance, cfg, save_scores):
#
...
def inference(cfg):
    ...
    with open(os.path.join(cfg.config_path, 'config.pkl'), 'rb') as f:
    ...
    checkpoint = torch.load(cfg.model_checkpoint_path)
    ...
    print('\nPerforming inference using model', cfg.model_checkpoint_path)
    ...
    #loss, acc, mAP_05, mAP_03 = infer(test_loader, test_set, model, device, model_instance, cfg, save_scores=True)


    #infer(dataloader, dataset, model, device, model_instance, cfg, save_scores):
